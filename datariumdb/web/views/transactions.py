...
from datariumdb.common.exceptions import SchemaValidationError, ValidationError
from datariumdb.models import Transaction
from datariumdb.web.views.base import make_error
from datariumdb.web.views import parameters
...
class TransactionApi(Resource):
    def get(self, tx_id):
        ...
        with pool() as datarium:
            tx, status = datarium.get_transaction(tx_id, include_status=True)

        if not tx or status is not datarium.TX_VALID:
...
class TransactionListApi(Resource):
    def get(self):
        ...
        with current_app.config['datarium_pool']() as datarium:
            txs = datarium.get_transactions_filtered(**args)
    ...
    def post(self):
        ...
        with pool() as datarium:
            datarium.statsd.incr('web.tx.post')
            try:
                datarium.validate_transaction(tx_obj)
            except ValidationError as e:
                return make_error(
                    400,
                    'Invalid transaction ({}): {}'.format(type(e).__name__, e)
                ...
                datarium.write_transaction(tx_obj)
