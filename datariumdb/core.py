...
from datariumdb import exceptions as core_exceptions
from datariumdb.common import crypto, exceptions
from datariumdb.common.utils import gen_timestamp, serialize

import datariumdb

from datariumdb import backend, config_utils, fastquery
from datariumdb.consensus import BaseConsensusRules
from datariumdb.models import Block, Transaction


class Datarium(object):
    """Datarium API
    ...
    def __init__(self, public_key=None, private_key=None, keyring=[], connection=None, backlog_reassign_delay=None):
        """Initialize the Datarium instance

        A Datarium instance has several configuration parameters (e.g. host).
        If a parameter value is passed as an argument to the Datarium
        ...
        its default value (defined in datariumdb.__init__).

        Args:
            public_key (str): the base58 encoded public key for the ED25519 curve.
            private_key (str): the base58 encoded private key for the ED25519 curve.
            keyring (list[str]): list of base58 encoded public keys of the federation nodes.
            connection (:class:`~datariumdb.backend.connection.Connection`):
                A connection to the database.
        """

        config_utils.autoconfigure()

        self.me = public_key or datariumdb.config['keypair']['public']
        self.me_private = private_key or datariumdb.config['keypair']['private']
        self.nodes_except_me = keyring or datariumdb.config['keyring']

        if backlog_reassign_delay is None:
            backlog_reassign_delay = datariumdb.config['backlog_reassign_delay']
        self.backlog_reassign_delay = backlog_reassign_delay

        consensusPlugin = datariumdb.config.get('consensus_plugin')
        ...
        self.connection = connection if connection else backend.connect(**datariumdb.config['database'])
        if not self.me or not self.me_private:
            raise exceptions.KeypairNotFoundException()

        self.statsd = statsd.StatsClient(datariumdb.config['graphite']['host'])

    federation = property(lambda self: set(self.nodes_except_me + [self.me]))
    """ Set of federation member public keys """

    def write_transaction(self, signed_transaction):
        """Write the transaction to datarium.

        When first writing a transaction to the datarium the transaction will be kept in a backlog until
    ...
    def get_transaction(self, txid, include_status=False):
        ...
        This query begins by looking in the datarium table for all blocks containing
        ...
        if blocks_validity_status:
            ...
            blocks_validity_status = {
                ...
                if status != Datarium.BLOCK_INVALID
            }
            if blocks_validity_status:
                ...
                for target_block_id in blocks_validity_status:
                    if blocks_validity_status[target_block_id] == Datarium.BLOCK_VALID:
        ...
        if response:
            if tx_status == self.TX_IN_BACKLOG:
                ...
                # If we are reading from the datarium collection the asset is
    ...
    def get_status(self, txid):
        """Retrieve the status of a transaction with `txid` from datarium.
    ...
    def get_blocks_status_containing_tx(self, txid):
        ...
        if blocks:
            ...
            if list(blocks_validity_status.values()).count(Datarium.BLOCK_VALID) > 1:
                block_ids = str([
                    block for block in blocks_validity_status
                    if blocks_validity_status[block] == Datarium.BLOCK_VALID
                ])
    ...
    def get_spent(self, txid, output):
        ...
        A transaction can be used as an input for another transaction. Datarium
        ...
        # checks if the datarium has any transaction with input {'txid': ...,
        # 'output': ...}
    ...
    def write_block(self, block):
        """Write a block to datarium.

        Args:
            block (Block): block to write to datarium.
        """
    ...
    def prepare_genesis_block(self):
        ...
        metadata = {'message': 'Hello World from the DatariumDB'}
    ...
    def create_genesis_block(self):
        ...
        Block created when datarium is first initialized. This method is not atomic, there might be concurrency
        problems if multiple instances try to write the genesis block when the DatariumDB Federation is started,
        but it's a highly unlikely scenario.
        """

        # 1. create one transaction
        # 2. create the block with one transaction
        # 3. write the block to the datarium
    ...
    def text_search(self, search, *, limit=0):
        ...
        #       See https://github.com/datariumdb/datariumdb/issues/1496
        for asset in assets:
