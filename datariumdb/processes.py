...
import datariumdb
from datariumdb import config_utils
from datariumdb.pipelines import vote, block, election, stale
from datariumdb.events import Exchange, EventTypes
from datariumdb.web import server, websocket_server
...
****************************************************************************
*                                                                          *
*   Initialization complete. DatariumDB Server is ready and waiting.       *
*   You can send HTTP requests via the HTTP API documented in the          *
*   DatariumDB Server docs at:                                             *
*    https://datariumdb.com/http-api                                       *
*                                                                          *
...
def start_events_plugins(exchange):
    plugins = config_utils.load_events_plugins(
        datariumdb.config.get('events_plugins'))
...
def start():
    logger.info('Initializing DatariumDB...')
    ...
    # start the web api
    app_server = server.create_server(datariumdb.config['server'])
    ...
    # start message
    logger.info(BANNER.format(datariumdb.config['server']['bind']))
