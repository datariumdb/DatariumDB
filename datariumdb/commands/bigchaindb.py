"""Implementation of the `datariumdb` command,
the command-line interface (CLI) for DatariumDB Server.
"""
...
from datariumdb.common import crypto
from datariumdb.common.exceptions import (StartupError,
                                          DatabaseAlreadyExists,
                                          KeypairNotFoundException,
                                          DatabaseDoesNotExist)
import datariumdb
from datariumdb import backend, processes
from datariumdb.backend import schema
from datariumdb.backend.admin import (set_replicas, set_shards, add_replicas,
                                      remove_replicas)
from datariumdb.backend.exceptions import OperationError
from datariumdb.commands import utils
from datariumdb.commands.messages import (
    CANNOT_START_KEYPAIR_NOT_FOUND,
    RETHINKDB_STARTUP_ERROR,
)
from datariumdb.commands.utils import (
    configure_datariumdb, start_logging_process, input_on_stderr)
...
def run_configure(args, skip_if_exists=False):
    ...
    """
    config_path = args.config or datariumdb.config_utils.CONFIG_DEFAULT_PATH
    ...
    conf = copy.deepcopy(datariumdb.config)

    # Patch the default configuration with the new values
    conf = datariumdb.config_utils.update(
            conf,
            datariumdb.config_utils.env_config(datariumdb.config))
    ...
    database_keys = datariumdb._database_keys_map[args.backend]
    conf['database'] = datariumdb._database_map[args.backend]
    ...
    if config_path != '-':
        datariumdb.config_utils.write_config(conf, config_path)
    else:
...
def run_export_my_pubkey(args):
    ...
    """
    pubkey = datariumdb.config['keypair']['public']
...
def run_drop(args):
    """Drop the database"""
    dbname = datariumdb.config['database']['name']
...
def run_start(args):
    ...
    logger.info('DatariumDB Version %s', datariumdb.__version__)

    if args.allow_temp_keypair:
        if not (datariumdb.config['keypair']['private'] or
                datariumdb.config['keypair']['public']):

            private_key, public_key = crypto.generate_key_pair()
            datariumdb.config['keypair']['private'] = private_key
            datariumdb.config['keypair']['public'] = public_key
        else:
    ...
    logger.info('Starting DatariumDB main process with public key %s',
                datariumdb.config['keypair']['public'])
    processes.start()
...
def create_parser():
    parser = argparse.ArgumentParser(
        description='Control your DatariumDB node.',
    ...
    # parser for starting DatariumDB
    start_parser = subparsers.add_parser('start',
                                         help='Start DatariumDB')
