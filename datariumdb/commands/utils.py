...
import datariumdb
import datariumdb.config_utils
from datariumdb import backend
from datariumdb.common.exceptions import StartupError
from datariumdb.log.setup import setup_logging
from datariumdb.version import __version__


def configure_datariumdb(command):
    """Decorator to be used by command line functions, such that the
    configuration of datariumdb is performed before the execution of
    the command.
    ...
    def configure(args):
        ...
        datariumdb.config_utils.autoconfigure(
...
def start_logging_process(command):
    ...
    .. important::
        ...
        state of :obj:`datariumdb.config` at the moment this decorator
        is invoked.

    """
    @functools.wraps(command)
    def start_logging(args):
        from datariumdb import config
...
def start_rethinkdb():
    ...
    Raises:
        :class:`~datariumdb.common.exceptions.StartupError` if
...
base_parser = argparse.ArgumentParser(add_help=False, prog='datariumdb')
