...
To see what this looks like in DatariumDB, first note that our backend interfaces have been
configured to dispatch based on a backend's **connection type**.

Call `datariumdb.backend.connect()` to create an instance of a `Connection`:

```python
from datariumdb.backend import connect
...
```python
from datariumdb.backend import query
...
DatariumDB will load and register the configured backend's implementation automatically (see
`datariumdb.backend.connect()`), so you should always just be able to call an interface function if
