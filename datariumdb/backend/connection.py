...
import datariumdb
from datariumdb.common.exceptions import ConfigurationError
from datariumdb.backend.exceptions import ConnectionError


BACKENDS = {
    'mongodb': 'datariumdb.backend.mongodb.connection.MongoDBConnection',
    'rethinkdb': 'datariumdb.backend.rethinkdb.connection.RethinkDBConnection'
}
...
def connect(backend=None, host=None, port=None, name=None, max_tries=None,
    ...
    Returns:
        An instance of :class:`~datariumdb.backend.connection.Connection`
    ...
    backend = backend or datariumdb.config['database']['backend']
    host = host or datariumdb.config['database']['host']
    port = port or datariumdb.config['database']['port']
    dbname = name or datariumdb.config['database']['name']
    ...
    replicaset = replicaset or datariumdb.config['database'].get('replicaset')
    ssl = ssl if ssl is not None else datariumdb.config['database'].get('ssl', False)
    login = login or datariumdb.config['database'].get('login')
    password = password or datariumdb.config['database'].get('password')
    ca_cert = ca_cert or datariumdb.config['database'].get('ca_cert', None)
    certfile = certfile or datariumdb.config['database'].get('certfile', None)
    keyfile = keyfile or datariumdb.config['database'].get('keyfile', None)
    keyfile_passphrase = keyfile_passphrase or datariumdb.config['database'].get('keyfile_passphrase', None)
    crlfile = crlfile or datariumdb.config['database'].get('crlfile', None)
    ...
    try:
        ...
        raise ConfigurationError('Backend `{}` is not supported. '
                                 'DatariumDB currently supports {}'.format(backend, BACKENDS.keys()))
...
class Connection:
    ...
    def __init__(self, host=None, port=None, dbname=None,
        ...
        dbconf = datariumdb.config['database']
