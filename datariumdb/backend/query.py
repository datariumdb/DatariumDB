...
def write_block(connection, block):
    """Write a block to the datarium table.
...
def get_block(connection, block_id):
    """Get a block from the datarium table.
...
def count_blocks(connection):
    """Count the number of blocks in the datarium table.
...
def text_search(conn, search, *, language='english', case_sensitive=False,
    ...
    raise OperationError('This query is only supported when running '
                         'DatariumDB with MongoDB as the backend.')
