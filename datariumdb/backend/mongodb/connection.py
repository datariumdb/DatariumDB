...
import datariumdb
from datariumdb.utils import Lazy
from datariumdb.common.exceptions import ConfigurationError
from datariumdb.backend.exceptions import (DuplicateKeyError,
                                           OperationError,
                                           ConnectionError)
from datariumdb.backend.connection import Connection
...
class MongoDBConnection(Connection):
    ...
    def __init__(self, replicaset=None, ssl=None, login=None, password=None,
        ...
        self.replicaset = replicaset or datariumdb.config['database']['replicaset']
        self.ssl = ssl if ssl is not None else datariumdb.config['database'].get('ssl', False)
        self.login = login or datariumdb.config['database'].get('login')
        self.password = password or datariumdb.config['database'].get('password')
        self.ca_cert = ca_cert or datariumdb.config['database'].get('ca_cert', None)
        self.certfile = certfile or datariumdb.config['database'].get('certfile', None)
        self.keyfile = keyfile or datariumdb.config['database'].get('keyfile', None)
        self.keyfile_passphrase = keyfile_passphrase or datariumdb.config['database'].get('keyfile_passphrase', None)
        self.crlfile = crlfile or datariumdb.config['database'].get('crlfile', None)
...
def initialize_replica_set(host, port, connection_timeout, dbname, ssl, login,
    ...
    host = '{}:{}'.format(datariumdb.config['database']['host'],
                          datariumdb.config['database']['port'])
    config = {'_id': datariumdb.config['database']['replicaset'],
