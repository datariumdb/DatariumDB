...
The value of each DatariumDB Server configuration setting is
...
CONFIG_DEFAULT_PATH = os.environ.setdefault(
    'DATARIUMDB_CONFIG_PATH',
    os.path.join(os.path.expanduser('~'), '.datariumdb'),
)

CONFIG_PREFIX = 'DATARIUMDB'
CONFIG_SEP = '_'
...
def env_config(config):
    ...
    The name of the env variable is built combining a prefix (``DATARIUMDB``)
    with the path to the value. If the ``config`` in input is:
    ``{'database': {'host': 'localhost'}}``
    this function will try to read the env variable ``DATARIUMDB_DATABASE_HOST``.
    """
...
def set_config(config):
    """Set datariumdb.config equal to the default config dict,
    then update that with whatever is in the provided config dict,
    and then set datariumdb.config['CONFIGURED'] = True
    ...
    Note:
        Any previous changes made to ``datariumdb.config`` will be lost.
    """
    # Deep copy the default config into datariumdb.config
    datariumdb.config = copy.deepcopy(datariumdb._config)
    # Update the default config with whatever is in the passed config
    update(datariumdb.config, update_types(config, datariumdb.config))
    datariumdb.config['CONFIGURED'] = True


def update_config(config):
    """Update datariumdb.config with whatever is in the provided config dict,
    and then set datariumdb.config['CONFIGURED'] = True
...
def write_config(config, filename=None):
    ...
    Args:
        ...
        filename (str): the name of the file that will store the new configuration. Defaults to ``None``.
            If ``None``, the HOME of the current user and the string ``.datariumdb`` will be used.
    """
    if not filename:
        filename = CONFIG_DEFAULT_PATH
...
def is_configured():
    return bool(datariumdb.config.get('CONFIGURED'))
...
def autoconfigure(filename=None, config=None, force=False):
    ...
    set_config(newconfig)  # sets datariumdb.config
...
def load_consensus_plugin(name=None):
    ...
    Returns:
        an uninstantiated subclass of ``datariumdb.consensus.AbstractConsensusRules``
    """
    if not name:
        return BaseConsensusRules

    # TODO: This will return the first plugin with group `datariumdb.consensus`
    ...
    plugin = None
    for entry_point in iter_entry_points('datariumdb.consensus', name):
        plugin = entry_point.load()

    # No matching entry_point found
    if not plugin:
        raise ResolutionError(
            'No plugin found in group `datariumdb.consensus` with name `{}`'.
            format(name))
    ...
    if not issubclass(plugin, (BaseConsensusRules,)):
        raise TypeError('object of type "{}" does not implement `datariumdb.'
...
def load_events_plugins(names=None):
    ...
    for name in names:
        for entry_point in iter_entry_points('datariumdb.events', name):
