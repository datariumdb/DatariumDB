"""Custom exceptions used in the `datariumdb` package.
"""
from datariumdb.exceptions import DatariumDBError


class ConfigurationError(DatariumDBError):
    """Raised when there is a problem with server configuration"""


class DatabaseAlreadyExists(DatariumDBError):
    """Raised when trying to create the database but the db is already there"""


class DatabaseDoesNotExist(DatariumDBError):
    """Raised when trying to delete the database but the db is not there"""


class StartupError(DatariumDBError):
    """Raised when there is an error starting up the system"""


class CyclicBlockchainError(DatariumDBError):
    """Raised when there is a cycle in the blockchain"""


class KeypairNotFoundException(DatariumDBError):
    """Raised if operation cannot proceed because the keypair was not given"""


class KeypairMismatchException(DatariumDBError):
    """Raised if the private key(s) provided for signing don't match any of the
    current owner(s)"""


class OperationError(DatariumDBError):
...
class ValidationError(DatariumDBError):
