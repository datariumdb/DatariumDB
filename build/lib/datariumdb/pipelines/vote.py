...
from datariumdb import backend, Datarium
from datariumdb.models import Transaction, Block, FastTransaction
...
class Vote:
    ...
    def __init__(self):
        ...
        # we need to create a temporary instance of DatariumDB that we use
        # only to query RethinkDB

        # This is the Datarium instance that will be "shared" (aka: copied)
        # by all the subprocesses

        self.datarium = Datarium()
        self.last_voted_id = Datarium().get_last_voted_block().id

        self.counters = Counter()
        self.blocks_validity_status = {}

        dummy_tx = Transaction.create([self.datarium.me],
                                      [([self.datarium.me], 1)]).to_dict()
        self.invalid_dummy_tx = dummy_tx

    def validate_block(self, block_dict):
        if not self.datarium.has_previous_vote(block_dict['id']):
            try:
                block = Block.from_db(self.datarium, block_dict, from_dict_kwargs={
            ...
            try:
                block._validate_block(self.datarium)
    ...
    def validate_tx(self, tx_dict, block_id, num_tx):
        ...
        try:
            ...
            new = self.datarium.is_new_transaction(tx.id, exclude_block_id=block_id)
            if not new:
    ...
    def vote(self, tx_validity, block_id, num_tx):
        ...
        if self.counters[block_id] == num_tx:
            vote = self.datarium.vote(block_id,
                                      self.last_voted_id,
    ...
    def write_vote(self, vote, num_tx):
        ...
        self.datarium.write_vote(vote)
        self.datarium.statsd.incr('pipelines.vote.throughput', num_tx)
        return vote
