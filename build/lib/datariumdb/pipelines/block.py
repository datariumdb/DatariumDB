...
from datariumdb.backend.changefeed import ChangeFeed
from datariumdb.models import Transaction
from datariumdb.common.exceptions import (ValidationError,
...
class BlockPipeline:
    ...
    def filter_tx(self, tx):
        ...
        Returns:
            ...
            ``None`` otherwise.
        """
        if tx['assignee'] == self.datarium.me:
            tx.pop('assignee')
    ...
    def validate_tx(self, tx):
        ...
        Returns:
            :class:`~datariumdb.models.Transaction`: The transaction if valid,
            ``None`` otherwise.
        """
        try:
        ...
        if not self.datarium.is_new_transaction(tx.id):
            self.datarium.delete_transaction(tx.id)
            return None
    ...
    def write(self, block):
        ...
        Args:
            block (:class:`~datariumdb.models.Block`): the block of
        ...
        self.datarium.write_block(block)
        self.datarium.statsd.incr('pipelines.block.throughput',
    ...
    def delete_tx(self, block):
        ...
        Args:
            block (:class:`~datariumdb.models.Block`): the block
                containg the transactions to delete.

        Returns:
            :class:`~datariumdb.models.Block`: The block.
        """
        self.datarium.delete_transaction(*[tx.id for tx in block.transactions])
        return block
...
def get_changefeed():
    connection = backend.connect(**datariumdb.config['database'])
