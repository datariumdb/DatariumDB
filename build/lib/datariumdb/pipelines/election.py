...
from datariumdb.models import Block
from datariumdb import Datarium
from datariumdb.events import EventTypes, Event
...
class Election:
    ...
    def check_for_quorum(self, next_vote):
        ...
        next_block = self.datarium.get_block(block_id)

        result = self.datarium.block_election(next_block)
        self.handle_block_events(result, block_id)
        if result['status'] == self.datarium.BLOCK_INVALID:
            return Block.from_dict(next_block)

        # Log the result
        if result['status'] != self.datarium.BLOCK_UNDECIDED:
    ...
    def requeue_transactions(self, invalid_block):
        ...
        for tx in invalid_block.transactions:
            self.datarium.write_transaction(tx)
    ...
    def handle_block_events(self, result, block_id):
        if self.events_queue:
            if result['status'] == self.datarium.BLOCK_UNDECIDED:
                return
            elif result['status'] == self.datarium.BLOCK_INVALID:
                event_type = EventTypes.BLOCK_INVALID
            elif result['status'] == self.datarium.BLOCK_VALID:
                event_type = EventTypes.BLOCK_VALID

            event = Event(event_type, self.datarium.get_block(block_id))
