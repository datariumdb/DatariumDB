...
from datariumdb import backend, utils
from datariumdb.backend.rethinkdb import changefeed
...
def get_transaction_from_block(connection, transaction_id, block_id):
    return connection.run(
            r.table('datarium', read_mode=READ_MODE)
            .get(block_id)
            .get_field('block')
...
def _get_asset_create_tx_query(asset_id):
    return r.table('datarium', read_mode=READ_MODE) \
...
def get_owned_ids(connection, owner):
    query = (r.table('datarium', read_mode=READ_MODE)
...
def get_block(connection, block_id):
    return connection.run(r.table('datarium').get(block_id))
...
def get_new_blocks_feed(connection, start_block_id):  # pragma: no cover
    ...
    for change in changefeed.run_changefeed(connection, 'datarium'):
