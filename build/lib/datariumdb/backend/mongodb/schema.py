...
def create_indexes(conn, dbname):
    create_datarium_secondary_index(conn, dbname)
...
def create_datarium_secondary_index(conn, dbname):
    logger.info('Create `datarium` secondary index.')

    # to order blocks by timestamp
    conn.conn[dbname]['datarium'].create_index([('block.timestamp',
                                                 ASCENDING)],
                                               name='block_timestamp')

    # to query the datarium for a transaction id, this field is unique
    conn.conn[dbname]['datarium'].create_index('block.transactions.id',
                                               name='transaction_id')

    # secondary index for asset uuid, this field is unique
    conn.conn[dbname]['datarium']\
