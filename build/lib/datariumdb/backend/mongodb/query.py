...
from datariumdb.backend.mongodb.changefeed import run_changefeed
from datariumdb.common.exceptions import CyclicBlockchainError
from datariumdb.common.transaction import Transaction
from datariumdb.backend.exceptions import DuplicateKeyError, OperationError
from datariumdb.backend.utils import module_dispatch_registrar
from datariumdb.backend.mongodb.connection import MongoDBConnection
...
def get_spent(conn, transaction_id, output):
    cursor = conn.run(
        conn.collection('datarium').aggregate([
            {'$match': {
...
def get_new_blocks_feed(conn, start_block_id):
    namespace = conn.dbname + '.datarium'
    ...
    feed = run_changefeed(conn, 'datarium', last_ts)
